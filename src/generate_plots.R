...
main <- function(prob_label, run_num) {
  ...
  d_np = rbind.data.frame(matrix_non_is, matrix_is, np_emb_non_is, np_emb_is) %>%
    dplyr::mutate(length = str_count(ConditionedTree, "\\w+"),
                  length_factorized = MAPScore / length)
...
## running into some problems when running all the FG learning in parallel.
## so going one at a time where the second arg is now the run number instead
